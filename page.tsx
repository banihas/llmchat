...
import { Ellipsis, Send, Sidebar, SquarePen, Trash2, WandSparkles, Copy } from "lucide-react";
...
export default function ChatPage() {
  ...
  const [showOptionsMenu, setShowOptionsMenu] = useState(false); // To show/hide the options menu
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === "/" && !showOptionsMenu) {
      setShowOptionsMenu(true);
      setFilteredOptions(options); // Show all options initially
    } else if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      handleSendMessage();
      setShowOptionsMenu(false); // Hide options menu when message is sent
    }
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (showOptionsMenu && value.includes("/")) {
      const query = value.split("/").pop() || "";
      const newFilteredOptions = options.filter(option => option.toLowerCase().includes(query.toLowerCase()));
      setFilteredOptions(newFilteredOptions);
    } else {
      setShowOptionsMenu(false); // Hide the menu if the `/` is not present
    }
  };
  ...
  const handleCopyMessage = (content: string) => {
    ...
    // Optionally, you can add a toast notification here to inform the user that the content has been copied
  };
  ...
  const handleOptionSelect = (option: string) => {
    const newValue = input.replace(/\/[a-zA-Z]*$/, option);
    setInput(newValue);
    setShowOptionsMenu(false); // Hide the options menu
  };
          ...
          <div className="w-[300px] border-r flex flex-col">
              ...
              {chats.length === 0 ? (
                ...
                chats.map((chat) => (
                  <div
                    key={chat.id}
                    className={cn(
                      "cursor-pointer p-2 rounded flex justify-between items-center group",
                      chat.id === currentChatId ? "bg-[#34C759] text-white hover:bg-[#2FB350]" : "hover:bg-neutral-700",
            ...
            {chats.length > 0 && (
              <div className="mt-auto p-4">
                <Button
                  className="w-full bg-[#007AFF] hover:bg-[#0056B3] text-white rounded-lg shadow-none border-none"
              ...
              <div className="flex flex-col gap-6">
                {messages.map((message) => (
                  ...
                  >
                    ...
                    <div className={cn("p-2 rounded-full break-words whitespace-pre-wrap", 
                      message.role === "user" ? "bg-[#007AFF] ml-auto px-4 text-white" : "max-w-[75%]")}>
                      {message.role === "assistant" ? (
                        <ReactMarkdown
                    ...
                    {message.role === "assistant" && hoveredMessageId === message.id && message.content && (
          ...
          <div className="flex justify-center mb-2 mt-12">
            <div className="relative max-w-[800px] w-full">
              <ReactTextareaAutosize
                ref={inputRef}
                className="resize-none rounded-lg p-4 pr-12 w-full focus:outline-none bg-[#E5F1FF] placeholder-[#007AFF]/50"
                rows={2}
                maxRows={14}
                value={input}
